...
from utils.urls import REDNOTE_IMAGE_DOWNLOAD_URL


class RednoteImageManager:
    def __init__(
        self,
        cache_dir: str,
        max_retries: int = 5,
        ...
        require_convert_url_domain: bool = True,
        timeout: int = 50,
        download_type: str = "image",
        ...
        self.require_convert_url_domain = require_convert_url_domain
        self.timeout = timeout
        self.max_file_size = max_file_size
        
        assert download_type in ("image", "video", "both"), f"Unsupported {download_type}"
        self.download_type = download_type
    
    
    @staticmethod
    def convert_url_domain(
        url: str,
    ):
        return url.replace("ci.xiaohongshu.com", REDNOTE_IMAGE_DOWNLOAD_URL)
    ...
    async def download_from_url(
        ...
        if os.path.exists(cache_path):
            return cache_path
        url = self.convert_url_domain(url=url) if self.require_convert_url_domain else url
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                tqdm.write(f"Successfully downloaded: {url} to {cache_path}")
                return cache_path
            
            except Exception as e:
                tqdm.write(
                    f"Attempt {attempt}/{self.max_retries} failed to download from {url}. Error: {str(e)}"
                )
                if attempt < self.max_retries:
                    time.sleep(self.retry_delay)
                else:
                    tqdm.write(f"All {self.max_retries} attempts failed for {url}.")
                    return None
    
    
    async def download_from_image_url(
        self,
        image: Dict,
    ):
        if image.get("status", "") == "valid" or image.get("status", "") == "invalid":
            return image
        
        file_id = image["fileId"].split("/")[-1]
        ...
        if not os.path.exists(cache_path):
            path = await self.download_from_url(url=image["url"], cache_path=cache_path)
            if path is None:
                image["path"] = ""
                image["status"] = "require_re_download"
        else:
            path = cache_path
        
        if image.get("status", "") in ("", "require_re_download"):
            path = await self.download_from_url(url=image["url"], cache_path=cache_path)
            if path is None:
                image["path"] = ""
                image["status"] = "invalid"
    ...
    async def download_from_video_url(
        self,
        video: Dict,
    ):
        if len(video) == 0:
        ...
        if video.get("status", "") == "valid" or video.get("status", "") == "invalid":
            return video
        
        file_id = video["noteId"].split("/")[-1]
        file_name = file_id + "_video.mp4"
        cache_path = os.path.join(self.cache_dir, file_name)
        
        if not os.path.exists(cache_path):
            path = await self.download_from_url(url=video["url"], cache_path=cache_path)
            if path is None:
                video["path"] = ""
                video["status"] = "require_re_download"
            else:
                video["path"] = path
                video["status"] = "valid"
                return video
        else:
            video["path"] = cache_path
            video["status"] = "valid"
            return video
        
        if video.get("status", "") in ("", "require_re_download"):
            path = await self.download_from_url(url=video["url"], cache_path=cache_path)
            if path is None:
                video["path"] = ""
                video["status"] = "invalid"
    ...
    async def download_from_web_video_url(
        self,
        video: Dict
    ):
        if len(video) == 0:
        ...
        if not os.path.exists(cache_path):
            try:
                ...
                head_res = http_session.head(video["url"], timeout=self.timeout)
                head_res.raise_for_status()
                
                # 验证文件大小
                content_length = int(head_res.headers.get('Content-Length', 0))
                if content_length > self.max_file_size:
                    raise ValueError(f"File size too large: {content_length / 1e6:.2f}MB")
                
                def _stream_write_file(response: requests.Response) -> None:
                    """流式写入文件"""
                    total_size = 0
                    st = time.time()
                    ...
                    with open(cache_path, 'wb') as f:
                        for chunk in response.iter_content(chunk_size=8192):
                            if chunk:
                                ...
                                # 定期记录进度
                                if time.time() - st > 1:
                                    logging.debug(f"Downloaded: {total_size / 1e6:.2f}MB")
                                    st = time.time()
                ...
                # 下载文件
                with http_session.get(video["url"], stream=True, timeout=self.timeout) as res:
                    res.raise_for_status()
                    _stream_write_file(res)
                logging.info(f"Download completed: {cache_path}")
    ...
    async def act(
        self,
        input_dict: Dict,
        input_key: str = "search_notes",
    ):
        async def process_note(note: dict):
            if self.download_type in ("image", "both"):
                images = note.get("images", [])
                note["images"] = await asyncio.gather(
                    *(self.download_from_image_url(image=image) for image in images)
                )
            if self.download_type in ("video", "both"):
                video = note.get("video")
                if video:
                    note["video"] = await self.download_from_video_url(video=video)
            return note
        
        notes = input_dict.get(input_key, [])
        # for note in notes:
        #     if self.download_type in ("image", "both"):
        #         for image in note["images"]:
        #             image = await self.download_from_image_url(image=image)
        #     if self.download_type in ("video", "both"):
        #         video = note["video"]
        #         video = await self.download_from_video_url(video=video)
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        input_dict = await service.act(
            ...
            input_key=f"search_result_turn_{turn_id}",
        )
        breakpoint()
        print(input_dict)
