...
@gin.configurable()
class DisentangledOutlineGeneratorBlueprint:
    def __init__(
        self,
        use_zh: bool = False,
        model_name: str = "",
        ...
        system_template_en_file: str = "DisentangledOutlineGeneratorBlueprintQA_EN.jinja2",
        system_template_zh_file: str = "DisentangledOutlineGeneratorBlueprintQA_ZH.jinja2",
        ...
        system_template_en_file_style: str = "DisentangledOutlineGeneratorBlueprintStyleQA_EN.jinja2",
        system_template_zh_file_style: str = "DisentangledOutlineGeneratorBlueprintStyleQA_ZH.jinja2",
    ...
    @staticmethod
    def divide_outline_into_chunks(
        outline: str
    ):
    ...
    def count_reference(
        ...
        cite_tags = re.findall(cite_pattern, outline)
        cite_count = len(cite_tags)
        
        # 2. 统计具体的引用数量（turn_1_xx格式）
        # 提取所有cite标签内的内容
        cite_content_pattern = r'<cite>(.*?)</cite>'
        cite_contents = re.findall(cite_content_pattern, outline)
        ...
        # 3. 统计唯一引用数量
        cite_count = len(all_references)
        unique_references = set(all_references)
        unique_count = len(unique_references)
        return cite_count, unique_count
...
"""
        ...
        cite_cnt, unique_cnt = self.count_reference(outline=response)
        input_dict[f"outline_turn_{turn_id}_cite_cnt"] = cite_cnt
        input_dict[f"outline_turn_{turn_id}_unique_cnt"] = unique_cnt
        return input_dict
...
if __name__ == "__main__":
    async def main():
        service = DisentangledOutlineGeneratorBlueprint(
            use_zh=True,
