...
from agent.OutlineGeneratorAgent import OutlineGenerator
...
async def run_agent(
    ...
    if not disable_outline_generator:
        ...
        if skip_outline_generator and os.path.exists(cache_path):
            ...
            while turn_id < max_outline_generator_turns:
                ...
                input_dict, turn_id = await outline_generator.act(
                    input_dict=input_dict,
                    turn_id=turn_id,
                )
                ...
                is_finish = input_dict[f"is_finish_turn_{turn_id}"]
                logging.info(f"finsh outline generator: {is_finish}")
            ...
            if use_hierarchical_writer:
                ...
                chunks = outline_generator.divide_outline_into_chunks(outline)
    ...
    if not disable_report_writer:
        ...
        if skip_report_writer and os.path.exists(cache_path):
            ...
            report_writer = ReportWriter(use_zh=use_zh, use_hierarchical_writer=use_hierarchical_writer)
            if use_hierarchical_writer:
    ...
    # logging.info(f"start output data at {output_path}")
    # with open(output_path, "w", encoding="utf-8") as f:
    #     json.dump(input_dict["outline"]["references"], f, ensure_ascii=False, indent=2)    
